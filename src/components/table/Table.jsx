import React, { useState } from 'react';
import "./table.scss";

import Table from '@material-ui/core/Table';
import TableBody from '@material-ui/core/TableBody';
import TableCell from '@material-ui/core/TableCell';
import TableContainer from '@material-ui/core/TableContainer';
import TableHead from '@material-ui/core/TableHead';
import TableRow from '@material-ui/core/TableRow';
import Paper from '@material-ui/core/Paper';



const List = () => {
    const rows = [
        {
            order: "#143567",
            date: "22/01/2021",
            customer: "Dominik Lamakani",
            total: "$129.00",
            status: "Refunded",
            items: 1,
            location: "Shanghai, CN",
            img: "https://flagpedia.net/data/flags/w580/cn.png",
            payment: "Subscription"
        },
        {
            order: "#227799",
            date: "22/01/2021",
            customer: "Mark Cameron",
            total: "$89.00",
            status: "Approved",
            items: 2,
            location: "New Mexico, MX",
            img: "https://cdn.britannica.com/73/2573-004-29818847/Flag-Mexico.jpg",
            payment: "Subscription"
        },
        {
            order: "#143567",
            date: "22/01/2021",
            customer: "Carolyn McNeail",
            total: "$89.00",
            status: "Approved",
            items: 2,
            location: "Milan, IT",
            img: "data:image/png;base64,",
            payment: "One-time"
        },
        {
            order: "#664491",
            date: "22/01/2021",
            customer: "Brian Halligan",
            total: "$59.00",
            status: "Pending",
            items: 1,
            location: "Shanghai, CN",
            img: "https://flagpedia.net/data/flags/w580/cn.png",
            payment: "One-time"
        },
        {
            order: "#780044",
            date: "22/01/2021",
            customer: "Cool Robot",
            total: "$39.00",
            status: "Refunded",
            items: 1,
            location: "Marseille, FR",
            img: "data:image/png;base64,",
            payment: "Subscription"
        },
        {
            order: "#786512",
            date: "21/01/2021",
            customer: "Sergio Gonnelli",
            total: "$59.00",
            status: "Approved",
            items: 1,
            location: "Bologna, IT",
            img: "data:image/png;base64,",
            payment: "One-time"
        },
        {
            order: "#664679",
            date: "21/01/2021",
            customer: "James Gill",
            total: "$89.00",
            status: "Approved",
            items: 1,
            location: "Paris, FR",
            img: "data:image/png;base64,",
            payment: "Subscription"
        },
        {
            order: "#112467",
            date: "21/01/2021",
            customer: "Alex Brooks",
            total: "$129.00",
            status: "Approved",
            items: 2,
            location: "London, UK",
            img: "https://upload.wikimedia.org/wikipedia/en/thumb/a/ae/Flag_of_the_United_Kingdom.svg/1200px-Flag_of_the_United_Kingdom.svg.png",
            payment: "Subscription",
        }
    ]

   
    
  return (
    <TableContainer component={Paper} className='table'>
      <Table >
        <TableHead>
          <TableRow>
            <TableCell className='tableCell'><input type="checkbox"/></TableCell>
            <TableCell className='tableCell'>ORDER</TableCell>
            <TableCell className='tableCell'>DATE</TableCell>
            <TableCell className='tableCell'>CUSTOMER</TableCell>
            <TableCell className='tableCell'>TOTAL</TableCell>
            <TableCell className='tableCell'>STATUS</TableCell>
            <TableCell className='tableCell'>ITEMS</TableCell>
            <TableCell className='tableCell'>LOCATION</TableCell>
            <TableCell className='tableCell'>PAYMENT OPTION</TableCell>
            
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row) => (
            <TableRow key={row.order}>
              <TableCell className='checkbox'><input type="checkbox"/></TableCell>
              <TableCell className='order'>
                  <span className= {`number ${row.order}`}>{row.order}</span>
                </TableCell>
              <TableCell className='date'>{row.date}</TableCell>
              <TableCell className='customer'>
                  <span className={`name ${row.customer}`}>{row.customer}</span>
              </TableCell>
              <TableCell className='total'>
                  <span className={`bill ${row.total}`}>{row.total}</span>
              </TableCell>
              <TableCell className='status'>
                  <span className={`color ${row.status}`}>{row.status}</span>
              </TableCell>
              <TableCell className='item'>{row.items}</TableCell>
              <TableCell className='location'>
                  <div className="flag">
                      <img src={row.img} alt="" srcset="" className='image'/>
                      {row.location}
                  </div>
              </TableCell>
              <TableCell className='payment'>{row.payment}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
    </TableContainer>
  )
}

export default List;
